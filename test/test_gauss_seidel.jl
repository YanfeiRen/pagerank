...
@testset "simple gs" begin
  ...
  x,lastiter = ManyPagerank.FastGaussSeidel(A, id, d, 0.85, size(A,1)-1)
  pr = seeded_pagerank(A,0.85,size(A,1)-1,1/1e6)
  @test lastiter != -1
  @test norm(pr./sum(pr)-x/sum(x),1) <= 1/1e6

  x,lastiter = ManyPagerank.FastGaussSeidelFromZero(A, id, d, 0.85, size(A,1)-1)
  @test lastiter != -1
  @test norm(pr./sum(pr)-x/sum(x),1) <= 1/1e6
end
...
@testset "multi gs" begin
  ...
  y,lastiter = ManyPagerank.GaussSeidelMultiPR(Pt, 0.85, SVector((9 : 16)...))
  @test lastiter != -1
  X = _unpack(y)
  ...
  y,lastiter = ManyPagerank.multi_gauss_seidel_from_zero(A, 0.85, SVector((9 : 16)...))
  @test lastiter != -1
  X = _unpack(y)
  PR = PR./sum(PR,dims=1)
  X = X./sum(X,dims=1)
